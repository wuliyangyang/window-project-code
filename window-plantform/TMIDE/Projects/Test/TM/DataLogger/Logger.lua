...
require("pathManager")
require("Global")
local locker = InstrumentLocker:new();
locker:createInstrumentLocker("lock")
...
function printer(...)
    ...
    local new_sendPivot_log = "C:\\vault\\Intelli_log\\DLog_Module"..tostring(args.module).."_Slot"..tostring(args.uut)..os.date("_%Y-%m-%d")..".txt";
...
-- logger = logging.file("C:\\vault\\Intelli_log\\DataLogger_log_Module"..tostring(args.module).."_Slot"..tostring(args.uut)..os.date("_%Y-%m-%d")..".log")
...
local log_path = "C:/vault/Intelli_log/CSV/";

local log_path_p = "C:/vault/Intelli_log/";
local pivot_csv_path = log_path_p.."Pivot_Log_Module"..tostring(args.module).."_Slot"..tostring(args.uut)..os.date("_%Y-%m-%d");

MakeNFolder(string.gsub(pivot_csv_path,"/","\\"));
MakeNFolder(string.gsub(log_path,"/","\\"));
...
-- package.path = package.path..";..\\TestEngine\\Driver\\?.lua"
package.cpath = package.cpath..";"..libDir
-- package.cpath = package.cpath..";..\\TestEngine\\lib\\?.dll"
printer("package.cpath : %s",package.cpath);
-- printer("package.path : %s",package.path);
printer("Current load lib path:%s",libDir)
printer("package.path : %s",package.path);

local sequence_sub, err = nil,nil;
local logger_pub = nil;

local jsonFilePath = CurrDir;
jsonFilePath = string.gsub(jsonFilePath,"DataLogger","TesterConfig")
printer(jsonFilePath)
...
--{"Function":"MtcpResult","Params":{"DutSN":"","SocketSN":"","ErrCode":0,"ErrStr":"","BinCode":55},"Slot":"CH1"};
--{"Function":"SequencerResult","Params":{"DutSN":"","SocketSN":"","ErrCode":0,"ErrStr":""},"Slot":"CH1"};
local function pubMsg(fun,data)
	local x = {}
	x["function"] = tostring(fun)
	data["dutSn"] = content_MLBSN;
	data["socketSn"] = content_SOCKETSN;
	x["params"] = data;
	x["slot"] = args.uut + args.module * args.slots;
	local msg = json.encode(x)
	if logger_pub then
		printer("<logger pub> : "..tostring(msg));
...
function CombineCSVHead()
	-- body
	local SoftWare_Version = "V0.1_20161022_M0"
	local csv_header_1 = ""..",".."Intelligent"..",".."SoftWare Version: "..""..","..""..","..SoftWare_Version.."\n";		--first Line
...
function isLogExisted(scriptName,dateTime)
	...
	local fullLogName = "Profile_"..realName.."_Module"..tostring(args.module).."_Slot"..tostring(args.uut).."_"..dateTime..log_file_type;
...
function IsLogAllExited(scriptName,dateTime)
	-- printer("scriptName : ",scriptName)
	local realName = string.match(tostring(scriptName),".+[/\\](.+)%.") or "Test_Script";
	local fullLogName = "Profile_"..realName.."All_Slot".."_"..dateTime..log_file_type;
	fullLogName = string.gsub(fullLogName, "[%*<>/|?:\"\\]*", "")
	log_file_full_path = log_path..fullLogName;
	local fileFlag = io.open(log_file_full_path, "r");
	if	fileFlag then
...
local function ResolveMessage(message)
	...
	elseif	msgType == MSG_SEQUENCE_END then
		...
		if tableOfKeyValue.data.result == 1 then
			msg_test_status = "PASS";
		else
			msg_test_status = "FAIL";
		end
		...
		local csv_content = content_MLBSN..",,,"..tostring(args.uut)..","..msg_test_status..","..msg_error..","..table.concat(items_listOfFail,";")..","..seq_starttime..","..seq_stoptime..","..totaltime..","..table.concat(items_value,",").."\n";		
		...
		locker:lockInstrument();
		--log all in one csv
		if	IsLogAllExited(script_Name,startdate) then			
			WriteLogContent(log_file_full_path,csv_content);
		else			
			local csv_header = CombineCSVHead();					
			WriteLogContent(log_file_full_path,csv_header..csv_content);		
		...
		locker:unlockInstrument();
		ClearContentMsg();
		return;

	elseif	msgType == MSG_ITEM_START then
		-- printer("Resolved MSG_ITEM_START");	
		-- content_itemstart = "";
		return;
...
local function addPoller()
	...
	poller:add(sequence_sub, zmq.POLLIN, function()
		...
		ResolveMessage(msg);
		-- local dispatch_status, result_value = pcall(ResolveMessage, msg);
  --       if dispatch_status==false then
  --       	local errorItem="error occur in item:"..tostring(msg[5]).."\n";
  --      		printer(errorItem);
  --      		printer("error msg is:"..tostring(result_value));
  --       end 
	end)
