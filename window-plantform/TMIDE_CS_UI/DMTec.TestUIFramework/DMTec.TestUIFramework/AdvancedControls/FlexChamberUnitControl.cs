...
namespace DMTec.TestUIFramework.AdvancedControls
{
    ...
    /// 
    /// </summary>
    public partial class FlexChamberUnitControl : UserControl
    {
        ...
        public FlexChamberUnitControl(int chamberNo)//
        {
            InitializeComponent();
        ...
        public FlexChamberUnitControl(ChamberConfig config)//
        {
            InitializeComponent();
            ...
            chamberSimple.event_StatisticsClear += OnStatisticsClear;
            chamberSimple.SetStatisticsClearButtonVisible(true);
            ...
            {
                this.tlp_Main.RowStyles[1].Height = 0; //删除第1行
            ...
            else
            {
                ...
                //myDutSnInputBox.event_SendSocketBarcode += OnNewSocketSNInputHandle;
                myDutSnInputBox.event_NewDutBarcode += OnNewDutSNInputHandle;
            }
        ...
        public void SetStatisticsClearButtonVisible(bool isV)
        {
            chamberSimple.SetStatisticsClearButtonVisible(isV);
        }
        ...
        //
        private MessageBoxEx myMessageBox;
        //
        ...
        public delegate void SequenceEndHandler(string dutSn, bool isPass);
        public event SequenceEndHandler event_End;

        public delegate void HighLightItemHandler(string item, bool isPass);
        public event HighLightItemHandler event_HighLightItem;
        ...
        /// <summary>
        /// The List load key words of test item who need to HightLight.
        /// </summary>
        public List<String> HighLightItemKeyWords
        {
            get { return _highLightItemKeyWords; }
            private set { _highLightItemKeyWords = value; }
        }private List<string> _highLightItemKeyWords;

        /// <summary>
        /// If highlight the specified item according to the key word(item.Tid).
        /// </summary>
        public bool IsHighLightItem { get; set; }
        ...
        {
            if(null != mySeqListener) mySeqListener.StartListen();
            if (null != myTMSyncConnector) myTMSyncConnector.Start();
            if (null != mySMConnector) mySMConnector.StartTest();
            if (null != myEngineConnector) myEngineConnector.StartListen();
        ...
        public void OnSeqEndHandle(object sender, SeqEndArgs arg)
        {
            ...
            NotifySequenceEndResult(arg.result);
        }

        private void NotifySequenceEndResult(string isPass)
        {
            if (null != event_End)
            {
                event_End.Invoke(DutSN, (Consts.TESTEND_PASS_STRING == isPass) ? true : false);
            }
        }
        ...
        public void OnSeqItemEndHandle(object sender, SeqItemEndArgs arg)
        {
            ...
            if(IsHighLightItem) HighLightItemHandle(arg);
        }
        ...
        private void HighLightItemHandle(SeqItemEndArgs item)
        {
            if (0 >= HighLightItemKeyWords.Count) return;

            foreach(string itemKeyWord in HighLightItemKeyWords)
            {
                if (item.tid == itemKeyWord)
                {
                    bool result = ("true" == item.result) ? true : false;
                    HighLightItem(itemKeyWord, result);
                    return;
                }
            }
        }
        ...
        private void HighLightItem(string item, bool isPass)
        {
            if(null != event_HighLightItem)
            {
                event_HighLightItem.BeginInvoke(item, isPass, null, null);
            }
        }
        ...
        public void RegisterSequenceEndHandle(SequenceEndHandler handler)
        {
            event_End += handler;
        }

        public void RegisterHighLightItemHandler(HighLightItemHandler handler)
        {
            event_HighLightItem += handler;
        }
        ...
        /// <summary>
        /// Add one test item who need to highlight. 
        /// </summary>
        /// <param name="keyWordString"></param>
        public void AddHighLightItem(string keyWordString)
        {
            if (string.IsNullOrEmpty(keyWordString.Trim())) return;
            HighLightItemKeyWords.Add(keyWordString);
        }

        /// <summary>
        /// Add some test items who need to highlight.
        /// </summary>
        /// <param name="keyWordStrings"></param>
        public void AddHightLightItems(List<string> keyWordStrings)
        {
            if (0 >= keyWordStrings.Count) return;
            foreach(string keyword in keyWordStrings)
            {
                AddHighLightItem(keyword);
            }
        }
...
//             str += myStatistics.PassRate.ToString() + "%";
        ...
        public void BindSeqListenerEvents(ref SequencerListener listener)
        {
           ...
           // listener.event_LogInfo += OnLogInfo;
            //listener.event_LogWarn += OnLogWarn;
            //listener.event_LogError += OnLogError;
            //listener.event_uencerMessage += SeqOriginalMsgHandle;   //Bind Msg Received Event
            //
            //Bind SeqStart Event
            listener.event_SeqStart  += OnSeqStartHandle;
            //Bind SeqEnd Event
            listener.event_SeqEnd += OnSeqEndHandle;

            //Bind ItemStart Event
            listener.event_ItemStart += OnSeqItemStartHandle;
            //Bind ItemEnd Event
            listener.event_ItemEnd   += OnSeqItemEndHandle;
            //Bind ErrorReport Event
            listener.event_ReportError += OnSeqReportErrHandle;
            //Bind UopDetect Event
            listener.event_UopDetect += OnSeqUopDetectHandle;
            //Bind Attribute Found Event
            listener.event_AttributeFound += OnSeqAttributeFoundHandle;
            //Bind HeartBeat Event
            //listener.event_HeartBeat += OnSeqHeartbeatHandle;
        }
        ...
        public void BindTMSyncConnectorEvents(ref TMSyncConnector tmsc)
        {
            ...
            //tmsc.event_Message    += OnTMSyncOriginalMsg;
            //tmsc.event_LogInfo        += OnTMSyncLogInfo;
            //tmsc.event_NewSN          += OnTMSyncNewSN;
            //tmsc.event_NewErrorReport += OnTMSyncNewErrorReport;
        }
        ...
        {
            //smc.event_OriginalMsg += OnSMOriginalMsg;
            //smc.event_LogInfo += OnSMInfo;
            //smc.event_LogWarn += OnSMWarn;
            //smc.event_LogError += OnSMError;
            //
        ...
        public void BindEngineListenerEvents(ref EngineConnector egnc)
        {
            ...
            //Component msg to ui display
            egnc.event_LogInfo     += OnLogInfo;
            egnc.event_LogWarn     += OnLogWarn;
            egnc.event_LogError    += OnLogError;
            //egnc.event_OriginalMsg += EngineOriginalMsgHandle;   //Bind Msg Received Event
            //Bind HeartBeat Event
           // egnc.event_Engine_HEARTBEAT   += EngineHeartbeatHandle;
        }
