...
#region // Seq SM Eng//
        private int InitSeqListeners(string[] addressList, out List<SequencerListener> seqListenerList)
        {
            ...
            for (int i = 0; i < addressList.Length; i++)
            {
                ...
                listener.event_SeqStart += Listener_evt_SeqStart;
                listener.event_SeqEnd += Listener_evt_SeqEnd;
        ...
        private int InitEngineConnectors(string[] addressList, out List<EngineConnector> engineConnectorList)
        {
            ...
            for (int i = 0; i < addressList.Length; i++)
            {
                ...
                listener.SubscribeAddress = addressList[i];
                //listener.IsOriginalMsgOutput = true;
                listener.event_LogInfo += LogInfo;
                listener.event_LogError += LogError;
                listener.event_LogWarn += LogWarn;
...
#region//Events Handler//
        ...
        {
            if (!myStationHeartbeatPanel.IsSMHBOK) return;

            myStateMachineConnectorList[0].StartTest(CheckEnableSlot());
...
#region //Others//
        private Dictionary<int, string> CheckEnableSlot()
        {
            ...
            {
                if (myStationHeartbeatPanel.seqStatus[i] == 0 && myStationHeartbeatPanel.engStatus[i]==0 && uutList[i].IsUUTEnable)
                {
                    dic.Add(i, uutList[i].DutSN);
                }
            }
