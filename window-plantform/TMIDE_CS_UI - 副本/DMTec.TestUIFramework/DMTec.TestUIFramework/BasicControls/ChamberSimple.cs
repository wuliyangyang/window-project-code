...
namespace DMTec.TestUIFramework.BasicControls
{
    public partial class ChamberSimple : UserControl
    {
        ...
        public event StatisticsClearHandler evt_StatisticsClear;
        ...
        public void BindSeqListenerEvents(ref SequencerListener listener)
        {
            ...
            //listener.evt_Print2UI += MsgPrint;
            //listener.evt_OriginalMsg += SeqOriginalMsgHandle;   //Bind Msg Received Event

            //Bind Seq Events
            listener.evt_SeqStart  += OnSeqStartHandle;  //Bind SeqStart Event
            listener.evt_SeqEnd    += OnSeqEndHandle;    //Bind SeqEnd Event

            //listener.evt_SeqItemStart += ItemStartHandle;
            listener.evt_SeqItemEnd   += OnSeqItemEndHandle;

            //listener.evt_SeqReportError += ReportErrHandle;
            //listener.evt_SeqUopDetect += UopDetectHandle;
            //listener.evt_SeqHeartBeat += HeartbeatHandle;

            listener.evt_SeqAttributeFound += OnSeqAttributeFoundHandle;//Bind Attribute Found Event
        }
        ...
        {
            //smc.evt_SMOriginalMsg += OnSMCOriginalMsg;
            //smc.evt_Info += OnSMInfo;
            //smc.evt_Warn += OnSMWarn;
            //smc.evt_Warn += OnSMError;
            //
        ...
        private void btn_ClearStatistics_Click(object sender, EventArgs e)
        {
            ...
            {
                if (null != evt_StatisticsClear)
                {
                    evt_StatisticsClear.BeginInvoke(this, new EventArgs(), null, null);
                }
            }
        }
