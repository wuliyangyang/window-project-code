...
namespace DMTec.TestUIFramework.AdvancedControls
{
    public partial class TesterSimple : UserControl
    {
        ...
        public void BindNetEvents(int chamberNo, ref SequencerListener listener)
        {
            if(1 == chamberNo)
            {
                //listener.evt_SeqStart += chamberSimple1.evt_SeqStart
                //listener.evt_SeqStart = new SeqStartHandler()
                listener.evt_SeqStart += chamberSimple1.OnSeqStartHandle;

                listener.evt_SeqEnd += chamberSimple1.OnSeqEndHandle;
                listener.evt_SeqItemStart += chamberSimple1.OnSeqItemStartHandle;
                listener.evt_SeqItemEnd += chamberSimple1.OnSeqItemEndHandle;
                listener.evt_SeqAttributeFound += chamberSimple1.OnSeqAttributeFoundHandle;
            }
            else if(2 == chamberNo)
            {
                listener.evt_SeqStart += chamberSimple2.OnSeqStartHandle;
                listener.evt_SeqEnd += chamberSimple2.OnSeqEndHandle;
                listener.evt_SeqItemStart += chamberSimple2.OnSeqItemStartHandle;
                listener.evt_SeqItemEnd += chamberSimple2.OnSeqItemEndHandle;
                listener.evt_SeqAttributeFound += chamberSimple2.OnSeqAttributeFoundHandle;
            }
            else
            {
                //
